...
def label2train(label, label_info):
    for i, j in label_info['label2train']:
        if j == 255:
            label[label[:, :, 0] == i] = [0, 0, 0]
        else:
            label[label[:, :, 0] == i] = label_info['palette'][j]
...
def one_hot_dice(label, label_info):
    ...
    for i, l2t in enumerate(label_info['label2train']):
        if l2t[1] == 255:
            rgb = [0, 0, 0]
            semantic_map[:, :, 19][label == l2t[0]] = 1
        else:
            semantic_map[:, :, l2t[1]][label == l2t[0]] = 1
    return semantic_map
